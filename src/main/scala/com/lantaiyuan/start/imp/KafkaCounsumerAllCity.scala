...
object KafkaCounsumerAllCity extends SparkStart {
      ...
      .foreachRDD(rdd => {
        ...
        rdd.foreachPartition(pat => {
          ...
          pat.foreach(pair => {
           ...
           /*   println(JSON.parseObject(pair._5.toString).get("runstatus"))*/
              ...
              println("new_____"+DateUtil.formatRealTime(time*1000))
              ...
              // 高频次的gps点，来源于ITS_Topic_GIS_HD(5s/次)  (1个) 存储20个点(经纬度+时间(时间戳)  json串)
                                  if (runStatus=="1" && pair._4.split("@")(0).toString =="ITS_Topic_GIS_HD"){
            ...
            if (pair._2 != "" || pair._3 != "") {
              ...
              if (pair._4.split("@")(1) == "old") {
                ...
                jedis.set("settle_travltime_" + pair._1, jobj.toString)
                println("settle_travltime_" + pair._1+"-------------"+jedis.get("settle_travltime_" + pair._1))
                jedis.expire("settle_travltime_" + pair._1, 24 * 60 * 3600)
              }
  ...
  def readStimeCluster(str: String, jedis: JedisCluster): String = {
    ...
    val result = jedis.get(str)
    if (result!=null){
       if (JSON.parseObject(result).get("start_time").toString.contains("-")){
         DateUtil.formatTimeStr2TimeStamp(JSON.parseObject(result).get("start_time").toString).toString
       }else{
         JSON.parseObject(result).get("start_time").toString
       }
    }else {
     ""
    }
